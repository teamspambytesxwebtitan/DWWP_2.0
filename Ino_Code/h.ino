...
// void drawLoader(int cx, int cy, int frame) {
    // display.drawRect(x, y, width, height, SSD1306_WHITE);  

    // int segmentWidth = 10;
    // int gap = 5;
    // int speed = 2;
    // int startX = (frame * speed) % (width + gap);

    // for (int i = startX - (segmentWidth + gap); i < width; i += segmentWidth + gap) {
    //     if (i > 0) {
    //         display.fillRect(x + i, y + 1, segmentWidth, height - 2, SSD1306_WHITE);
    //     }
    // }

//   int radius = 8;  // Distance from center

//   // 🔹 8 predefined positions in a circular motion
//   int8_t pointsX[8] = {0,  7, 10,  7,  0, -7, -10, -7};
//   int8_t pointsY[8] = {-10, -7,  0,  7, 10,  7,   0, -7};

//   for (int i = 0; i < 8; i++) {
//     int alpha = (frame == i) ? WHITE : BLACK;  // 🔹 Highlight 1 dot at a time
//     display.fillCircle(cx + pointsX[i], cy + pointsY[i], 2, alpha);
//   }
// }
